...
# Industrial-grade Regex Patterns for Discord Strike
PATTERNS = {
    "Discord API Paths": r'/(?:api/v[0-9]|v[0-9]/[\w\-\/]+)',
    "Gateway/Websocket Signatures": r'[\"\'](?:wss://|gateway\.discord\.gg|gateway|DISPATCH|HEARTBEAT|IDENTIFY)[\"\']',
    "Internal Feature Flags": r'[\"\'](?:Experimental|Staff|Internal|DiscordInternal)[\w\-]+[\"\']',
...
async def fetch_and_mine(client: httpx.AsyncClient, url: str) -> dict:
    try:
        ...
        if not url.startswith("http"):
             url = "https://discord.com" + (url if url.startswith("/") else "/" + url)
...
async def main():
    console.print("[bold cyan]💀 AURA DEEP MINE: DISCORD STRIKE (PHASE 6)[/bold cyan]")
    
    if not os.path.exists("discord_bundles.txt"):
        console.print("[red]Error: discord_bundles.txt not found. Run hunt_discord.py first.[/red]")
        return
    ...
    # Final Output
    output_path = "discord_phase6_recon.json"
