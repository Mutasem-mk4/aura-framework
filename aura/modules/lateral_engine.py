...
class LateralEngine:
    ...
    async def _identify_environment(self, finding):
        ...
        try:
            return self.brain.reason_json(prompt).strip()
        except:
            return "LOCAL"
    ...
    async def _extract_identities(self, finding, env):
        ...
        if env == "AWS":
            console.print("[red][!] Nebula: Attempting IAM Role extraction from Metadata v1/v2...[/red]")
            # Logic to generate SSRF payload for AWS metadata
        elif env == "K8S":
            console.print("[red][!] Nebula: Probing Service Account tokens in /var/run/secrets/...[/red]")
            # Logic to generate RCE payload for secret extraction
