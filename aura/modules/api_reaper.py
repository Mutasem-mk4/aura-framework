...
class APIReaper:
    ...
    def __init__(self, target: str = None, session=None, discovered_endpoints: list = None):
        # Ensure target always has a protocol prefix for urljoin to work correctly
    ...
    async def run(self):
        ...
        common_paths = [
            "/swagger.json", "/api/swagger.json", "/openapi.yaml",
        ...
        if not self.endpoints and self.discovered_endpoints:
            console.print(
                f"[yellow][!] No Swagger found. Building API map from "
                f"{len(self.discovered_endpoints)} Frontend Deconstructor paths...[/yellow]"
            )
    ...
    async def _extrapolate_mobile_routes(self):
        ...
        console.print(
            f"[bold purple][👻 MOBILE GHOST] Extrapolating hidden backends "
            f"from {len(self.endpoints)} base routes...[/bold purple]"
        )
        extrapolated = []

        mobile_patterns = [
            "/mobile", "/m", "/api/mobile", "/api/m",
            "/internal", "/api/internal", "/v1/mobile", "/v2/mobile"
        ]
        ...
        for ep in base_routes:
            ...
            base_authority = (
                ...
                if parsed.scheme else self.target
            )
            ...
            for pattern in mobile_patterns:
                ...
                try:
                    ...
                    if resp and resp.status_code in [200, 401, 403, 405]:
                        console.print(
                            f"[bold green][✓] GHOST BACKEND DISCOVERED: "
                            f"{new_url} ({resp.status_code})[/bold green]"
                        )
                        extrapolated.append({
                            "path": new_url,
                            "method": "GET",
        ...
        console.print(
            f"[bold green][✓] Extrapolation Complete: "
            f"Added {len(extrapolated)} ghost routes to audit queue.[/bold green]"
        )
    ...
    async def _parse_spec(self, spec: dict, base_url: str):
        ...
        console.print(
            f"[bold green][✓] API Mapping Complete: "
            f"{len(self.endpoints)} endpoints identified.[/bold green]"
        )
    ...
    async def audit_all(self, session_b=None):
        ...
        all_findings = []
        console.print(
            f"[bold cyan][⚔️ API REAPER] Launching Offensive Audit "
            f"on {len(self.endpoints)} routes...[/bold cyan]"
        )
    ...
    async def _test_bola(self, ep: dict, session_b):
        ...
        id_params = [
            ...
            if any(x in k.lower() for x in ["id", "uuid", "uid"])
        ]
        if not id_params:
            return None
                ...
                if resp and resp.status_code == 200:
                    ...
                    if baseline and baseline.status_code in [403, 404]:
                        ...
                        return {
                            ...
                            "url": ep["path"],
                            "content": (
                                f"User B successfully accessed User A's resource using "
                                f"id={original_val} while baseline id={random_id} was blocked."
                            )
                        }
            except Exception:
                pass
